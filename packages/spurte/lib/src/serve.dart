...
class SpurteServerResult {
  ...
  const SpurteServerResult._({required this.close, required HttpServer server})
      : _server = server;
...
class SpurteServer {
  final Future<SpurteServerResult> Function(int port,
      {void Function()? onListen, void Function()? onEnd}) listen;
...
Handler _clientHandler(DartDevcFrontendServerClient client,
    {String entrypoint = "web/main.dart"}) {
...
Future<SpurteServer> serve(ServerOptions options, {bool log = false}) async {
  ...
  String relativeEntry = p.isRelative(options.entry)
      ? options.entry
      : p.relative(options.entry, from: options.cwd);
  ...
  // https
  final chain = options.cert == null
      ? null
      : Platform.script.resolve(options.cert ?? "").toFilePath();
  final key = options.key == null
      ? null
      : Platform.script.resolve(options.key ?? "").toFilePath();
  ...
  return SpurteServer._(
      ...
      listen: (int port,
          {void Function()? onListen, void Function()? onEnd}) async {
        final server = await io.serve(handler, options.host, port,
        ...
        // autocompression?
        //
        return SpurteServerResult._(
            close: (bool force, {void Function()? onClose}) {
...
Future<Cascade> buildServer(DartClientResult devClient, String relativeEntry,
    ServerOptions options) async {
  bool dev = devClient is DartDevClientResult;
  var cascade = Cascade();
  if (dev) {
    cascade = cascade
        .add(_clientHandler(devClient.client, entrypoint: relativeEntry));
  }
  // serve public dir
  cascade = cascade.add((request) {
    ...
    final actualPath = p.join(options.publicDir,
        path.replaceFirst('/${options.publicDir}', '').replaceFirst('/', ''));
  ...
  if (dev) {
    cascade = cascade.add((req) {
      if (req.url.path == '/' || req.url.path == '') {
        ...
        indexHtmlContents = addScriptsToHtml(
            indexHtmlContents, [hmr_client_script],
            module: true);

        return Response.ok(indexHtmlContents,
  ...
  if (dev) {
    cascade = cascade
        .add(createFileHandler(devClient.dartSdk,
            url: p.join(p.dirname(relativeEntry), 'dart_sdk.js')))
        .add(createFileHandler("${devClient.dartSdk}.map",
            url: p.join(p.dirname(relativeEntry), 'dart_sdk.js.map')))
