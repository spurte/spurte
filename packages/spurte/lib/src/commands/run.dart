...
class RunCommand extends SpurteCommand {
  ...
  RunCommand() {
    argParser
      ...
      ..addFlag(
        'repl',
        help:
            'Whether to include a REPL for controlling the app from the command line',
        negatable: true,
        defaultsTo: true,
      )
  ...
  FutureOr? run() async {
    ...
    final webServer = await server.listen(serverOptions.port,
...
Future<bool> launchBrowser(String url) async {
  ...
  // check for safari
  if (Platform.isMacOS) {
    final safari = await Process.start('open', ['-a', 'Safari', url],
  ...
  // check for firefox
  final firefox =
      await Process.start('firefox', [url], mode: ProcessStartMode.detached);
